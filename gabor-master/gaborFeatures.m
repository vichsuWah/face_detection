function [featureVector, max_eyemapT] = gaborFeatures(img,gaborArray,d1,d2)
...
%         imshow(abs(gaborResult{i,j}),[]);
        gabor_sum(4*(i-1) + j) = sum(abs(gaborResult{i, j}), 'all');
    end
end
[~, I] = max(gabor_sum);
tmp = mod(I, 4);
...
max_eyemapT = gaborResult{fix((I-1)/4)+1, tmp};